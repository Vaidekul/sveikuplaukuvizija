...
class PayseraAdminHtml
{
    ...
    public function getMultipleSelectInput(array $options, array $selected): string
    {
        ...
        foreach ($options as $key => $option) {
            $html .= '<option value="' . $key . '" ' . ((in_array($key, $selected) === true) ? 'selected' : '')
                . '>' . $option . '</option>';
        }
    ...
    public function prepareEnableHtml(string $parametersName, string $parameterValue): string
    {
        return  '<select name="' . $parametersName . '">'
            . '<option value="yes">' . __('Enabled', PayseraPaths::PAYSERA_TRANSLATIONS) . '</option>'
            . '<option value="no" ' . (($parameterValue === 'no') ? 'selected' : '')
            . '>' . __('Disabled', PayseraPaths::PAYSERA_TRANSLATIONS) . '</option></select>';
    }
