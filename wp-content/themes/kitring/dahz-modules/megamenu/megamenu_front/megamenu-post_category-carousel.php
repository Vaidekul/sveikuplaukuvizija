	...
	<ul class="uk-slider-items uk-child-width-1-<?php echo esc_attr( $column ); ?> uk-grid" data-uk-grid>
		<?php
			foreach( $query as $term ) {
				...
				$img_id = dahz_framework_get_term_meta( $term->taxonomy, $term->term_id, 'image_upload', '' );

				$category_image = wp_get_attachment_image( $img_id, 'large', array( 'class' => 'images-slick' ) );
				...
				<li>
					<a href="<?php echo esc_url( $the_link ); ?>">
						<?php echo apply_filters( 'dahz_framework_megamenu_post_category_image', $category_image );?>
					</a>
