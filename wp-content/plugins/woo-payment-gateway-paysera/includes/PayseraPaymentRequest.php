...
class PayseraPaymentRequest
{
    private $payseraPaymentSettings;

    public function __construct()
    {
        $this->payseraPaymentSettings = new PayseraPaymentSettings();
    }
    ...
    {
        $settings = $this->payseraPaymentSettings->getPaymentPluginSettings();
        ...
        return [
            'projectid' => $this->limitIntLength($settings->getProjectId(), 11),
            'sign_password' => $this->limitStringLength($settings->getProjectPassword()),
            'orderid' => $this->limitIntLength($order->get_id(), 40),
            'amount' => $this->limitIntLength(intval(number_format((float) $order->get_total(), 2, '', '')), 11),
            ...
            'callbackurl' => $this->limitStringLength(
            ...
            'test' => $this->limitIntLength((int) $settings->isTestModeEnabled(), 1),
            'buyer_consent' => $this->limitIntLength((int) $settings->isBuyerConsentEnabled(), 1),
        ];
    }
