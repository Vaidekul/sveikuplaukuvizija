	...
	class Envato_Theme_Setup_Wizard {
		...
		public function get_logo_image() {
			...
			if ( $logo_image_id ) {
				...
				$image_url = get_template_directory_uri() . '/assets/images/logo/default-logo.svg';
			}
		...
		public function init_globals() {
			...
			$this->page_slug       = apply_filters( 'dahz_framework_setup_wizard_page_slug', 'kitring-setup' );
		...
		public function admin_menus() {
			...
			if ( $this->is_submenu_page() ) {
				...
				$add_subpage_function( $this->parent_slug, esc_html__( 'Setup Wizard', 'kitring' ), esc_html__( 'Setup Wizard', 'kitring' ), 'manage_options', $this->page_slug, array(
					$this,
					'setup_wizard',
				) );
			} else {
				add_theme_page( esc_html__( 'Setup Wizard', 'kitring' ), esc_html__( 'Setup Wizard', 'kitring' ), 'manage_options', $this->page_slug, array(
					$this,
					'setup_wizard',
				) );
			}
		...
		public function setup_wizard() {
			...
			wp_localize_script( 'envato-setup', 'envato_setup_params', array(
				...
				'verify_text'      => esc_html__( '...verifying', 'kitring' ),
			) );
		...
		public function setup_wizard_header() {
			...
			<head>
				...
				echo '<t';
				echo 'itle>' . esc_html__( 'Theme &rsaquo; Setup Wizard', 'kitring' ) . '</ti' . 'tle>'; ?>
			...
			<h1 id="wc-logo">
				<a href="<?php echo get_home_url();?>" target="_blank"><?php
					$image_url = $this->get_logo_image();
					if ( $image_url ) {
						$image = '<img class="site-logo" src="%s" alt="%s" style="max-width:%s; height:auto" />';
						printf(
							$image,
							get_template_directory_uri() . '/assets/images/logo/default-logo.svg',
							get_bloginfo( 'name' ),
			...
			*/
			...
			<?php if ( 'next_steps' === $this->step ) : ?>
				...
				href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard', 'kitring' ); ?></a>
			<?php endif; ?>
			</body>
			<?php
		...
		public function envato_setup_introduction() {
			...
			if ( false && isset( $_REQUEST['debug'] ) ) {
				...
				<p><?php esc_html_e( 'It looks like you may have recently upgraded to this theme. Great! This setup wizard will help ensure all the default settings are correct. It will also show some information about your new website and support options.', 'kitring' ); ?></p>
				...
				<ul>
					<li>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						   class="button-primary button button-next button-large"><?php esc_html_e( 'Run Setup Wizard Again', 'kitring' ); ?></a>
					</li>
					<li>
						<form method="post">
							<input type="hidden" name="reset-font-defaults" value="yes">
							<input type="submit" class="button-primary button button-large button-next"
							       value="<?php esc_attr_e( 'Reset font style and colors', 'kitring' ); ?>" name="save_step"/>
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
					   class="button button-large"><?php esc_html_e( 'Cancel', 'kitring' ); ?></a>
				</p>
				<?php
			} else {
				?>
		...
		public function envato_setup_introduction_save() {
			...
			if ( ! empty( $_POST['reset-font-defaults'] ) && $_POST['reset-font-defaults'] == 'yes' ) {
				...
				?>
				<p>
					<strong><?php esc_html_e( 'Options have been reset. Please go to Appearance > Customize in the WordPress backend.', 'kitring' ); ?></strong>
				</p>
				<?php
				return true;
			}
		...
		public function _get_plugins() {
			...
			foreach ( $instance->plugins as $slug => $plugin ) {
				if ( $instance->dahz_framework_check_plugin_active( $slug ) && false === $instance->does_plugin_have_update( $slug ) ) {
		...
		public function envato_setup_default_plugins() {
			...
			?>
			<h1><?php esc_html_e( 'Default Plugins', 'kitring' ); ?></h1>
			<form method="post">
				...
				if ( count( $plugins['all'] ) ) {
					?>
					<p><?php esc_html_e( 'Your website needs a few essential plugins. The following plugins will be installed or updated:', 'kitring' ); ?></p>
					...
					echo '<p><strong>' . esc_html_e( 'Good news! All plugins are already installed and up to date. Please continue.', 'kitring' ) . '</strong></p>';
				} ?>

				<p><?php esc_html_e( 'You can add and remove plugins later on from within WordPress.', 'kitring' ); ?></p>

				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"
					   data-callback="install_plugins"><?php esc_html_e( 'Continue', 'kitring' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
		...
		public function ajax_plugins() {
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No Slug Found', 'kitring' ) ) );
			}
			$json = array();
			...
			foreach ( $plugins['activate'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2'       => - 1,
						'message'       => esc_html__( 'Activating Plugin', 'kitring' ),
					);
					break;
				}
			}
			foreach ( $plugins['update'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2'       => - 1,
						'message'       => esc_html__( 'Updating Plugin', 'kitring' ),
					);
					break;
				}
			}
			foreach ( $plugins['install'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2'       => - 1,
						'message'       => esc_html__( 'Installing Plugin', 'kitring' ),
					);
					break;
				}
			}
			...
			if ( $json ) {
				...
				wp_send_json( array( 'done' => 1, 'message' => esc_html__( 'Success', 'kitring' ) ) );
			}
			exit;
		...
		public function envato_setup_default_content() {
			...
			<form method="post">
				<?php if ( $this->is_possible_upgrade() ) { ?>
					<p><?php esc_html_e( 'It looks like you already have content installed on this website. If you would like to install the default demo content as well you can select it below. Otherwise just choose the upgrade option to ensure everything is up to date.', 'kitring' ); ?></p>
				<?php } else { ?>
					<p><?php printf( esc_html__( 'It\'s time to insert some default content for your new WordPress website. Choose what you would like inserted below and click Continue. It is recommended to leave everything selected. Once inserted, this content can be managed from the WordPress admin dashboard. ', 'kitring' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
				<?php } ?>
		...
		public function _imported_term_id( $original_term_id, $new_term_id = false ) {
		...
		public function _imported_post_id( $original_id = false, $new_id = false ) {
		...
		public function _post_orphans( $original_id = false, $missing_parent_id = false ) {
		...
		public function _cleanup_imported_ids() {
			// loop over all attachments and assign the correct post ids to those attachments.

		}

		public $delay_posts = array();

		public function _delay_post_process( $post_type, $post_data ) {
		...
		public function _parse_gallery_shortcode_content($content){
		...
		public function _handle_post_orphans() {
		...
		public function _handle_delayed_posts( $last_delay = false ) {
		...
		public function _fetch_remote_file( $url, $post ) {
			// extract the file name and extension from the url
			$file_name  = basename( $url );
			$local_file = trailingslashit( get_template_directory() ) . '/dahz-framework/admin/envato_setup/content/images/' . $file_name;
			$upload     = false;
			...
			if ( ! $upload || $upload['error'] ) {
				...
				if ( is_array( $response ) && ! empty( $response['body'] ) && $response['response']['code'] == '200' ) {
					...
					return new WP_Error( 'import_file_error', esc_html__( 'Remote server did not respond', 'kitring' ) );
				}
				...
				if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
					...
					return new WP_Error( 'import_file_error', esc_html__( 'Remote file is incorrect size', 'kitring' ) );
				}

				if ( 0 == $filesize ) {
					@unlink( $upload['file'] );

					return new WP_Error( 'import_file_error', esc_html__( 'Zero size file downloaded', 'kitring' ) );
				}

				if ( ! empty( $max_size ) && $filesize > $max_size ) {
					@unlink( $upload['file'] );

					return new WP_Error( 'import_file_error', sprintf( esc_html__( 'Remote file is too large, limit is %s', 'kitring' ), size_format( $max_size ) ) );
				}
			}
		...
		public function envato_setup_color_style() {
            ...
            <form method="post">
                ...
                <div class="theme-presets">
                    <ul>
	                    ...
	                    foreach ( $this->site_styles as $style_name => $style_data ) {
		                    ?>
                            <li class="<?php echo esc_attr( $style_name == $current_style ? 'current' : '' );?>">
				...
				<?php $studiotracking_envato_link = 'studiotracking.envato.com/aff_c?offer_id=4&aff_id=1564&source=DemoInstall';?>
                <p><em>Please Note: Advanced changes to website graphics/colors may require extensive PhotoShop and Web
                        Development knowledge. We recommend hiring an expert from <a
                                href="<?php echo esc_url( $studiotracking_envato_link );?>"
                                target="_blank">Envato Studio</a> to assist with any advanced website changes.</em></p>
                <div style="display: none;">
                    <img src="<?php echo esc_url( 'studiotracking.envato.com/aff_i?offer_id=4&aff_id=1564&source=DemoInstall' );?>"
                         width="1" height="1"/>
                </div>
		...
		public function envato_setup_logo_design() {
			...
			?>
			<h1><?php esc_html_e( 'Logo', 'kitring' ); ?></h1>
			<form method="post">
			<?php $studio_envato_link = 'studiotracking.envato.com/aff_c?offer_id=4&aff_id=1564&source=DemoInstall';?>
				<p><?php printf( esc_html__( 'Please add your logo below. For best results, the logo should be a transparent PNG ( 466 by 277 pixels). The logo can be changed at any time from the Appearance > Customize area in your dashboard. Try %sEnvato Studio%s if you need a new logo designed.', 'kitring' ), '<a href="' . esc_url( $studio_envato_link ) . '" target="_blank">', '</a>' ); ?></p>
				...
				<table>
					<tr>
						...
						<td>
							<a href="#" class="button button-upload"><?php esc_html_e( 'Upload New Logo', 'kitring' ); ?></a>
						</td>
					</tr>
				</table>
				<?php $studio_envato_link = 'studiotracking.envato.com/aff_c?offer_id=4&aff_id=1564&source=DemoInstall';?>
				<p><em>Please Note: Advanced changes to website graphics/colors may require extensive PhotoShop and Web
						Development knowledge. We recommend hiring an expert from <a
							href="<?php echo esc_url( $studio_envato_link );?>"
							target="_blank">Envato Studio</a> to assist with any advanced website changes.</em></p>
				<div style="display: none;">
					<img src="<?php echo esc_url( 'studiotracking.envato.com/aff_i?offer_id=4&aff_id=1564&source=DemoInstall' );?>"
					     width="1" height="1"/>
		...
		public function envato_setup_updates() {
			?>
			<h1><?php esc_html_e( 'Theme Updates', 'kitring' ); ?></h1>
			<?php if ( function_exists( 'envato_market' ) ) { ?>
				<form method="post">
					...
					if ( count( $my_items ) ) {
						...
						<p class="envato-setup-actions step">
							<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
							   class="button button-large button-next button-primary"><?php esc_html_e( 'Continue', 'kitring' ); ?></a>
						</p>
						<?php
					} else {
						?>
						<p><?php esc_html_e( 'Please login using your ThemeForest account to enable Theme Updates. We update themes when a new feature is added or a bug is fixed. It is highly recommended to enable Theme Updates.', 'kitring' ); ?></p>
						<p>When an update becomes available it will show in the Dashboard with an option to install.</p>
						<p>
							<em>On the next page you will be asked to Login with your ThemeForest account and grant
								permissions to enable Automatic Updates. If you have any questions please <a
									href="#" target="_blank">contact us</a>.</em>
						</p>
						<p class="envato-setup-actions step">
							<input type="submit" class="button-primary button button-large button-next"
							       value="<?php esc_attr_e( 'Login with Envato', 'kitring' ); ?>" name="save_step"/>
							<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
							   class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'kitring' ); ?></a>
							<?php wp_nonce_field( 'envato-setup' ); ?>
						</p>
		...
		public function envato_setup_customize() {
			...
			<p>
				<em>Advanced Users: If you are going to make changes to the theme source code please use a <a
						href="<?php echo esc_url( 'codex.wordpress.org/Child_Themes' );?>" target="_blank">Child Theme</a> rather than
			...
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
				   class="button button-primary button-large button-next"><?php esc_html_e( 'Continue', 'kitring' ); ?></a>
			</p>
		...
		public function envato_setup_help_support() {
			?>
			<h1><?php esc_html_e( 'Help and Support', 'kitring' );?></h1>
			...
			<p>Item Support can be accessed from <a href="<?php echo esc_url( 'http://support.daffyhazan.com/' );?>" target="_blank"><?php echo esc_html( 'support.daffyhazan.com/' );?></a>
				and includes:</p>
			...
			<p>More details about item support can be found in the ThemeForest <a
					href="<?php echo esc_url( 'themeforest.net/page/item_support_policy' );?>" target="_blank">Item Support Polity</a>. </p>
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
				   class="button button-primary button-large button-next"><?php esc_html_e( 'Agree and Continue', 'kitring' ); ?></a>
				<?php wp_nonce_field( 'envato-setup' ); ?>
			</p>
			<?php
		}
		...
		public function envato_setup_ready() {
			...
			<h1><?php esc_html_e( 'Your Website is Ready!', 'kitring' ); ?></h1>
			...
			<p>Please come back and <a href="<?php echo esc_url( 'www.themeforest.net/downloads' );?>" target="_blank">leave a 5-star rating</a>
				if you are happy with this theme. <br/>Follow <a href="<?php echo esc_url( 'www.themeforest.net/user/dahz' );?>" target="_blank">@dahz</a>
				on ThemeForest to see updates. Thanks! </p>

			<div class="envato-setup-next-steps">
				<div class="envato-setup-next-steps-first">
					<h2><?php esc_html_e( 'Next Steps', 'kitring' ); ?></h2>
					<ul>
						<li class="setup-product"><a class="button button-primary button-large"
						                             href="<?php echo esc_url( 'www.themeforest.net/user/dahz' );?>"
						                             target="_blank"><?php esc_html_e( 'Follow @dahz on ThemeForest', 'kitring' ); ?></a>
						</li>
						<li class="setup-product"><a class="button button-next button-large"
						                             href="<?php echo esc_url( home_url() ); ?>"><?php esc_html_e( 'View your new website!', 'kitring' ); ?></a>
						</li>
					</ul>
				</div>
				<div class="envato-setup-next-steps-last">
					<h2><?php esc_html_e( 'More Resources', 'kitring' ); ?></h2>
					<ul>
						<li class="howto"><a href="<?php echo esc_url( 'www.wordpress.org/support/' );?>"
						                     target="_blank"><?php esc_html_e( 'Learn how to use WordPress', 'kitring' ); ?></a>
						</li>
						<li class="rating"><a href="<?php echo esc_url( 'www.themeforest.net/downloads' );?>"
						                      target="_blank"><?php esc_html_e( 'Leave an Item Rating', 'kitring' ); ?></a></li>
						<li class="support"><a href="<?php echo esc_url( 'http://support.daffyhazan.com/' );?>"
						                       target="_blank"><?php esc_html_e( 'Get Help and Support', 'kitring' ); ?></a></li>
					</ul>
				</div>
			</div>
			<?php
		...
		public static $_current_manage_token = false;

		public function _manage_oauth_token( $token ) {
		 ...
		 * @since    1.1.4
		 */
		public function _array_merge_recursive_distinct( $array1, $array2 ) {
			$merged = $array1;
