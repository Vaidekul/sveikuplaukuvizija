...
class Breadcrumb_Trail {
	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			$breadcrumb .= '<ul class="trail-items uk-breadcrumb uk-margin-remove-bottom" itemscope itemtype="http://schema.org/BreadcrumbList">';
			...
			$breadcrumb = sprintf(
				'<%1$s role="navigation" aria-label="%2$s" class="breadcrumb-trail breadcrumbs uk-margin uk-margin-top" itemprop="breadcrumb">%3$s%4$s%5$s</%1$s>',
		...
		echo apply_filters( 'dahz_framework_breadcrumb_trail_html', $breadcrumb );
	}
	...
	protected function set_labels() {
		...
		$defaults = array(
			'browse'              => esc_html__( 'Browse:',                               'kitring' ),
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'kitring' ),
			'home'                => esc_html__( 'Home',                                  'kitring' ),
			'error_404'           => esc_html__( '404 Not Found',                         'kitring' ),
			'archives'            => esc_html__( 'Archives',                              'kitring' ),
			// Translators: %s is the search query.
			'search'              => esc_html__( 'Search results for: %s',                'kitring' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s',                               'kitring' ),
			// Translators: %s is the page number.
			'paged_comments'      => esc_html__( 'Comment Page %s',                       'kitring' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s',                             'kitring' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s',                               'kitring' ),
	...
	protected function add_network_home_link() {
		...
		if ( is_multisite() && !is_main_site() && true === $this->args['network'] )
			$this->items[] = sprintf( '<a class="uk-link" href="%s" rel="home">%s</a>', esc_url( network_home_url() ), $this->labels['home'] );
	}
	...
	protected function add_site_home_link() {
		...
		$this->items[] = sprintf( '<a class="uk-link" href="%s"%s>%s</a>', esc_url( user_trailingslashit( home_url() ) ), $rel, $label );
	}
	...
	protected function add_blog_items() {
		...
		if ( is_paged() )
			$this->items[] = sprintf( '<a class="uk-link" href="%s">%s</a>', esc_url( get_permalink( $post_id ) ), $title );
	...
	protected function add_singular_items() {
		...
		if ( $post_title = single_post_title( '', false ) ) {
			...
			if ( ( 1 < get_query_var( 'page' ) || is_paged() ) || ( get_option( 'page_comments' ) && 1 < absint( get_query_var( 'cpage' ) ) ) )
				$this->items[] = sprintf( '<a class="uk-link" href="%s">%s</a>', esc_url( get_permalink( $post_id ) ), $post_title );
	...
	protected function add_term_archive_items() {
				...
				if ( isset( $matches ) ) {
					...
					foreach ( $matches as $match ) {
						...
						if ( !empty( $post_types ) ) {
							...
							$this->items[] = sprintf( '<a class="uk-link" href="%s">%s</a>', esc_url( get_post_type_archive_link( $post_type_object->name ) ), $label );
		...
		if ( false === $done_post_type && 1 === count( $taxonomy->object_type ) && post_type_exists( $taxonomy->object_type[0] ) ) {
			...
			if ( 'post' === $taxonomy->object_type[0] ) {
				...
				if ( 'posts' !== get_option( 'show_on_front' ) && 0 < $post_id )
					$this->items[] = sprintf( '<a class="uk-link" href="%s">%s</a>', esc_url( get_permalink( $post_id ) ), get_the_title( $post_id ) );
		...
		if ( is_paged() )
			$this->items[] = sprintf( '<a class="uk-link" href="%s">%s</a>', esc_url( get_term_link( $term, $term->taxonomy ) ), single_term_title( '', false ) );
	...
	protected function add_post_type_archive_items() {
		...
		if ( is_paged() || is_author() )
			$this->items[] = sprintf( '<a class="uk-link" href="%s">%s</a>', esc_url( get_post_type_archive_link( $post_type_object->name ) ), post_type_archive_title( '', false ) );
	...
	protected function add_user_archive_items() {
		...
		if ( is_paged() )
			$this->items[] = sprintf( '<a class="uk-link" href="%s">%s</a>', esc_url( get_author_posts_url( $user_id ) ), get_the_author_meta( 'display_name', $user_id ) );
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'kitring' ) ) );
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'kitring' ) ) );
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'kitring' ) ) );
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'kitring' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'kitring' ) ) );
		$day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'kitring' ) ) );

		// Add the year and month items.
		$this->items[] = sprintf( '<a class="uk-link" href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y' ) ) ), $year );
		$this->items[] = sprintf( '<a class="uk-link" href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) ), $month );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'kitring' ) ) );
		$week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'kitring' ) ) );
	...
	protected function add_search_items() {
		...
		if ( is_paged() )
			$this->items[] = sprintf( '<a class="uk-link" href="%s">%s</a>', esc_url( get_search_link() ), sprintf( $this->labels['search'], get_search_query() ) );
	...
	protected function add_post_parents( $post_id ) {
		...
		while ( $post_id ) {
			...
			$parents[] = sprintf( '<a class="uk-link" href="%s">%s</a>', esc_url( get_permalink( $post_id ) ), get_the_title( $post_id ) );
	...
	protected function add_post_hierarchy( $post_id ) {
		...
		if ( $post_type_object->has_archive ) {
			...
			$this->items[] = sprintf( '<a class="uk-link" href="%s">%s</a>', esc_url( get_post_type_archive_link( $post_type ) ), $label );
		}
	...
	protected function add_post_terms( $post_id, $taxonomy ) {
		...
		if ( $terms && ! is_wp_error( $terms ) ) {
			...
			$this->items[] = sprintf( '<a class="uk-link" href="%s">%s</a>', esc_url( get_term_link( $term, $taxonomy ) ), $term->name );
		}
	}
	...
	function add_term_parents( $term_id, $taxonomy ) {
		...
		while ( $term_id ) {
			...
			$parents[] = sprintf( '<a class="uk-link" href="%s">%s</a>', esc_url( get_term_link( $term, $taxonomy ) ), $term->name );
	...
	protected function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				$this->items[] = sprintf( '<a class="uk-link" href="%s">%s</a>', esc_url( get_category_link( $categories[0]->term_id ) ), esc_html( $categories[0]->name ) );
