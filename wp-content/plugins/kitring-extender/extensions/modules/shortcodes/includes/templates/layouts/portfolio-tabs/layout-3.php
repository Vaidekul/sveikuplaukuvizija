		...
		<div <?php
			...
			<?php if ( has_post_thumbnail( get_the_ID() ) ) : ?>
				<a class="de-ratio de-ratio-<?php echo esc_attr( $media_ratio );?>" href="<?php echo esc_url( get_permalink() );?>">
					...
					<div class="de-sc-portfolio-tabs__overlay uk-position-cover uk-flex uk-flex-center uk-flex-middle uk-overlay uk-transition-scale-down">
						<i style="color: <?php echo esc_attr( $icon_color ) ?>" data-uk-icon="icon:plus;ratio:3"></i>
					</div>
				</a>
			<?php endif;?>
			<?php if ( empty( $is_hide_category ) && empty( $is_hide_title ) ) : ?>
			<div class="de-sc-portfolio-tabs__content uk-padding uk-padding">
				<?php
				if ( empty( $is_hide_title )  ) {
